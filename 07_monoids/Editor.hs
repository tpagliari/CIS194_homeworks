...
instance Applicative (Editor b) where
  pure a = Editor (pure a)
  Editor f <*> Editor x = Editor (f <*> x)

-- Add explicit instance declarations instead of using deriving
instance Monad (Editor b) where
  Editor m >>= f = Editor $ m >>= \a -> case f a of Editor n -> n

instance MonadIO (Editor b) where
  liftIO = Editor . liftIO

instance MonadState (b,Int) (Editor b) where
  get = Editor get
  put = Editor . put
